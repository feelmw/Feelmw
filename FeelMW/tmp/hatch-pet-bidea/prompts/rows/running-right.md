Create one horizontal animation strip for Codex pet `bidea`, state `running-right`.
...
Identity: same pet in every frame: Compact friendly non-animal desk companion inspired by logistics planning and route optimization: rounded teal body, small amber high-visibility vest accents, tiny clipboard charm, curious focused face, maker-workbench warmth, no logos or readable text.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `3d-toy`: Stylized 3D toy mascot with smooth rounded forms, simple materials, clear silhouette, and no photoreal complexity. User style notes: polished tiny vinyl toy mascot, readable whole-body silhouette, practical but warm, not overly cute, no text, no logos.
